...
目前使用的 Gradle 8.14.5 + AGP 8.13.2 编译，运行良好

新增 v3/v4 签名算法支持，修复一些警告、弃用以及报错~
...
修改项目中对应的文件。Gradle如果无法同步，可能需要开启网络代理
...
// ...
repositories {
    // 在 repositories 节点添加 mavenCentral 仓库
    mavenCentral()
}
// ...
```
...
andResGuard {
    ...
    onlyV3V4Sign = boolean    // 仅使用 v3/v4 签名
    delMetaInf = boolean      // 删除 apk 的 META-INF 目录，因为删除操作在 apk 签名之前，所以不会影响 v1 签名

   //  注意：此节点已经移除，像我一样注释掉。插件会自动寻找对应平台的 7Zip 可执行文件
   // sevenzip {
...
```properties
# 可能需要禁用资源优化，生成的 APK 才能正常工作
android.enableResourceOptimizations=false

# 可能需要禁用 Gradle 守护进程，否则部分编译时生成的文件会被占用 (Windows)
...
编译完成终端会打印 ```output: 路径``` 指向最终生成的apk文件
